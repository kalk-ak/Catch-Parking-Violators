...
class User:
   ...
   User Class has the following attributes 
    ...
    '''
    def __init__(self, first: str, last: str, email: str, number: int, licence: str, id = None) -> None:
        '''
        ...
        self.email = email
        self.number = number 
        self.licence = []; self.licence.append(licence)
        self.licence_status = []
        self.licence_type = []
...
transaction_data = pd.read_csv("~/Desktop/python_script/data/transaction_data.csv", encoding='UTF-16', low_memory=False)
enterprise_subscription_data = pd.read_csv("~/Desktop/python_script/data/enterprise_subscription_detail.csv", encoding="UTF-16")
...
enterprise_subscription_data = enterprise_subscription_data[
    (enterprise_subscription_data["Enterprise Name"] == "Kellogg Square Residents - 1 st Vehicle") |
    (enterprise_subscription_data["Enterprise Name"] == "Kellogg Square Residents -Additional Vehicle")                       
]
enterprise_subscription_data = enterprise_subscription_data.reset_index(drop=True)

# Filter out the transient data
transaction_data = transaction_data[transaction_data["Subscription Status"] != "Transient"]
transaction_data = transaction_data.reset_index(drop=True)

# Filter out the non useful columns
transaction_data = transaction_data[
    (transaction_data["Site Internal Name"] == "Kellogg Square Reserved Nest (Minneapolis") |
...
def add_user(user_list, this_user) -> int:
    ...
    '''
    licence_plate = this_user.licence[0]        # There is only 1 licece plate and it is indexed in the first position
...
for i in range(len(enterprise_subscription_data)):
    ...
    if pd.isna(enterprise_subscription_data["User Email"][i]):          # If email cell is None
            ...
            for index in range(len(transaction_data)):
                  ...
                  if (transaction_data["Vehicle License Plate"][index] == enterprise_subscription_data["Vehicle License Plate Text"][i]):
                        current_user = User(
                            ...
                            licence = enterprise_subscription_data["Vehicle License Plate Text"][i].strip(),
...
print(f"Unknown User (no email or ID number) - not in the excel  = {no_email_users  - id_users}")



# Writing the organized list to an excel file
# Initialize an empty DataFrame
out_excel = pd.DataFrame(columns=["First", "Last", "Email", "License", "ID", "Phone Number", "Duplicate License Plate"])
...
for i in range(len(user_list)):
    for j in range(i + 1, len(user_list)):
        ...
        if (user_list[i].email != None) and (user_list[i].email == user_list[j].email):         # Get the duplicate email
...
out_excel.to_excel(f"~/Desktop/python_script/output/Organized Enterprise Subscription.xlsx")
print()
print(f"There are {num_duplicate_email} duplicate Email\nThere are {num_duplicate_number} duplicate Phone Number\nThere are {num_duplicate_licence} duplicate Licence Plate")

# Print the index of the duplicate license
if num_duplicate_licence > 0:
    print(f"\033[37m................\033[0m")  # White color for dots

    for pair in index_of_duplicate_licence:
        print(f"There is a duplicate license in index >>> \033[31m{pair[0]}, {pair[1]}\033[0m")  # Red color for indices
...
# Writing the organized list to an excel file
# Initialize an empty DataFrame
out_excel = pd.DataFrame(columns=["First", "Last", "Email", "License", "ID", "Phone Number", "License Status", "License Type"])
...
for i in range(len(user_list)):
    ...
    out_excel.loc[i, "ID"] = user_list[i].id
    out_excel.loc[i, "Phone Number"] = user_list[i].number
    out_excel.loc[i, "License Status"] = ", ".join(map(str, user_list[i].licence_status))
    out_excel.loc[i, "License Type"] = ", ".join(map(str, user_list[i].licence_type))

out_excel.to_excel(f"~/Desktop/python_script/output/Licence Status.xlsx")
...
for user in user_list:
    ...
    if user.email != None:      # Handles the case for users with email
        ...
        first_transaction_frequency = [0 for i in range(len(user.first_only_licence))]              # Populates the frequency list for all the 
        ...
        additional_transaction_frequency = [0 for i in range(len(user.additional_only_licence))]
...
output = pd.DataFrame(columns=["Email", "Visit Start Date", "Visit Start Time", "Visit End Date", "Visit Exit Time", "Visit Duration (minutes)", "License Plate", "User Id", "Flagged", "No First", "Total"])
for user in user_list:
    ...
    if user.email != None:                      # handles the case for only users with email
        ...
        for i in range(len(user.licence)):
            licence_plate = user.licence[i]             # Used to find the licence vehicle in transaction   
            ...
            for index in range(len(transaction_data)):
                ...
                if (licence_plate == transaction_licence):
                    visit_start_date = transaction_data["Visit Start Date (local)"][index]
                    visit_start_time = transaction_data["Visit Start Time (local)"][index]
                    ...
                    if (licence_plate != user.first_licence) and (licence_plate != user.additional_licence):
                        flag = "Flagged"
                    ...
                    output.loc[len(output)] = [user.email, visit_start_date, visit_start_time, visit_end_date, visit_end_time, visit_duration, licence_plate, user_id, flag, first_flag, total]
...
output.to_excel(f"~/Desktop/python_script/output/Final report.xlsx")
